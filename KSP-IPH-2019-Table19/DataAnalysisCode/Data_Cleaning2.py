...
df=combined_df[['Unit_ID','Beat_ID','lat','long','Collision_Type','Accident_Spot','NoOfVehicle_Involved','Junction_Control',
              'Road_Character','Road_Type','Surface_Type','Surface_Condition','Road_Condition',
              'weather','Main_Cause','Hit_Run','Holiday']]            
...
for Unit_ID in combined_df.Unit_ID.unique():
    ...
    for Cluster in perge.Cluster.unique():
        ...
        ClusterDf=ClusterDf.append(Cluster_perge)

#for Column_M in MasterColumns:
#    ClusterDf = ClusterDf.replace({Column_M: Master_filtered_dict})

ClusterDf.sort_values(by=['Unit_ID','Cluster_Score'],ascending=[True,False],inplace=True)
   
#for Unit_ID in ClusterDf.Unit_ID.unique():
#    df_Prevention=ClusterDf[ClusterDf.Unit_ID==Unit_ID]
#    df_Prevention.sort_values(by=['Unit_ID','Cluster_Score'],ascending=[True,False],inplace=True)
#    for Cluster in df_Prevention.Cluster:
#        df_Prevention.grou
